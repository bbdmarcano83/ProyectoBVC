...
Usa NAV/patrimonio, descuento a NAV/libro, rentabilidad, distribuciones y
consistencia. Las cifras monetarias venezolanas se normalizan a USD con la capa
BCV cuando existe metadata FX validada; nunca se inventan tasas históricas.
"""
...
def _percentile(rows: list[dict], key: str, higher: bool = True) -> dict[str, float]:
    ...
    if len(vals) == 1: return {pairs[0][0]: 50.0}
    out = {}
...
def _weighted(parts: list[tuple[float | None, float]]) -> tuple[float | None, float]:
    ...
    weight = sum(w for _, w in available); total = sum(w for _, w in parts)
    return round(sum(float(v) * w for v, w in available) / weight, 1), round(weight / total * 100.0, 1)


def _vehicle_metrics(data: dict) -> dict:
    fx, fx_meta = normalize_to_usd(data)
    market_cap = prefer_usd(fx, "market_cap")
...
def enrich_investment_vehicles(rows: list[dict]) -> tuple[list[dict], dict]:
    fundamentals, _ = load_fundamentals(); vehicle_rows: list[dict] = []
    for row in rows:
        src = get_source(str(row.get("simbolo") or ""))
        if not src or src.get("industry_type") != "investment_vehicle": continue
        data = fundamentals.get(str(row.get("simbolo") or "").upper())
        if data is None: data = fundamentals.get(str(src.get("canonical_symbol") or "").upper())
        row["industry_type_v5"] = "investment_vehicle"
        if not data:
            row["vehicle_score_v5"] = None; row["vehicle_coverage_v5"] = 0.0; continue
        row.update(_vehicle_metrics(data)); vehicle_rows.append(row)

    specs = {
        "vehicle_discount_to_nav_pct_v5": True, "vehicle_pb_v5": False,
        "vehicle_roa_pct_v5": True, "vehicle_distribution_yield_pct_v5": True,
        "vehicle_positive_income_periods_pct_v5": True, "vehicle_nav_cagr_pct_v5": True,
    }
    ranks = {k: _percentile(vehicle_rows, k, high) for k, high in specs.items()}
    for row in vehicle_rows:
        sym = str(row.get("simbolo")); r = lambda key: ranks[key].get(sym)
        score, coverage = _weighted([
            (r("vehicle_discount_to_nav_pct_v5"), .25), (r("vehicle_pb_v5"), .15),
            (r("vehicle_roa_pct_v5"), .20), (r("vehicle_distribution_yield_pct_v5"), .15),
            (r("vehicle_positive_income_periods_pct_v5"), .15), (r("vehicle_nav_cagr_pct_v5"), .10),
        ])
        row.update({
            "vehicle_score_v5": score, "vehicle_coverage_v5": coverage,
            "greenblatt_score_v5": None, "graham_score_v5": None, "buffett_score_v5": None,
            "fundamental_score_v5": score, "fundamental_coverage_v5": coverage,
            "fundamental_method_v5": "investment_vehicle_nav_value_quality_usd_bcv",
        })
    return rows, {
        ...
        "method": "USD/BCV NAV-book discount + ROA + distributions + consistency",
    }
